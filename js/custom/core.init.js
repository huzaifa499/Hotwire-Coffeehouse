...
function hotcoffee_ready_actions() {
    ...
    jQuery('.woocommerce,.woocommerce-page').on('click', '.mode_buttons a', function(e) {
        ...
        jQuery.cookie('hotcoffee_shop_mode', mode, {
            expires: 365,
            path: '/'
        });
        ...
        if (!exists) {
            var message_popup = hotcoffee_message_dialog('<label for="bookmark_title">' + strings['bookmark_title'] + '</label><br><input type="text" id="bookmark_title" name="bookmark_title" value="' + title + '">', strings['bookmark_add'], null,
                function(btn, popup) {
                    ...
                    jQuery.cookie('hotcoffee_bookmarks', JSON.stringify(list), {
                        expires: 365,
                        path: '/'
                    });
                    setTimeout(function() {
                        hotcoffee_message_success(strings['bookmark_added'], strings['bookmark_add']);
                    }, HOTCOFFEE_STORAGE['message_timeout'] / 4);
                });
        } else
...
function hotcoffee_init_appended_isotope(posts_container, filters) {
    ...
    if (!hotcoffee_check_images_complete(posts_container) && HOTCOFFEE_STORAGE['isotope_init_counter']++ < 30) {
        setTimeout(function() {
            hotcoffee_init_appended_isotope(posts_container, filters);
        }, 200);
        return;
    }
    // Add filters
...
function hotcoffee_init_media_elements(cont) {
    if (HOTCOFFEE_STORAGE['media_elements_enabled'] && cont.find('audio,video').length > 0) {
        if (window.mejs) {
            ...
            setTimeout(function() {
                hotcoffee_init_media_elements(cont);
            }, 400);
    }
}
...
function hotcoffee_comments_validate(form) {
    ...
    var error = hotcoffee_form_validate(form, {
        ...
        rules: [{
                ...
                min_length: {
                    value: 1,
                    message: strings['name_empty']
                },
                max_length: {
                    value: 60,
                    message: strings['name_long']
                }
            },
            {
                field: 'email',
            ...
            },
            {
                ...
                max_length: {
                    value: HOTCOFFEE_STORAGE['comments_maxlength'],
                    message: strings['text_long']
                }
            }
        ]
    });
...
function hotcoffee_login_validate(form) {
    ...
    var error = hotcoffee_form_validate(form, {
        ...
        rules: [{
                ...
                min_length: {
                    value: 1,
                    message: strings['login_empty']
                },
                max_length: {
                    value: 60,
                    message: strings['login_long']
                }
            },
            {
                field: "pwd",
                min_length: {
                    value: 4,
                    message: strings['password_empty']
                },
                max_length: {
                    value: 30,
                    message: strings['password_long']
                }
            }
        ]
    });
        ...
        rules: [{
            ...
            },
            {
                ...
                equal_to: {
                    ...
                    message: strings['password_not_equal']
                }
            }
        ]
    });
